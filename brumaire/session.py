...
class Game:
    ...
    def __init__(
        self, board_num: int, agents: List[AgentBase], log_enabled: bool = False
    ) -> None:
        assert len(agents) == 5
    ...
    def decide_napoleon(self) -> None:
        ...
        for player in range(5):
            ...
            self.log(
                lambda idx: f"player{player} declared {card_to_str(declarations[idx, player, [0, 1]], False)}."
            )
        ...
        self.log(
            lambda idx: "The napoleon will be selected randomly."
            if random_napoleon[idx]
            else "There is a valid goal."
        )

        randomly_selected_napoleon = np.repeat(random_napoleon[:, None], 5, axis=1) & (
        ...
        for idx in range(self.board_num):
            rolled_declarations[idx] = np.roll(
                declarations[idx], -first_player[idx], axis=0
            )

        declaration_indexes = (
            rolled_declarations[:, :, 1] * 4 + rolled_declarations[:, :, 0]
        ) * 5 - np.repeat(np.arange(5)[None, :], self.board_num, axis=0)
        ...
        for idx in range(self.board_num):
            ...
            declarations[idx, napoleon[idx], 1] = max(
                13, declarations[idx, napoleon[idx], 1]
            )
            ...
            adjutant_card = self.board.cards[
        ...
        self.log(
            lambda idx: f"The adjutant card is {card_to_str(declarations[idx, napoleon[idx], [2, 3]])}."
        )
    ...
    def discard_additional_cards(self) -> None:
        ...
        if self.log_enabled:
            ...
            for idx in range(self.board_num):
                discarded[idx] = discarded_cards_list[
                    discarded_cards_list[:, 0] == idx, 1
                ]
            for card_idx in range(4):
                self.log(
                    lambda idx: f"player{napoleons[idx]} discards {card_to_str(np.array([discarded[idx, card_idx] // 13, discarded[idx, card_idx] % 13]))}."
                )
    ...
    def trick(self, turn_num: int) -> None:
        ...
        for player_index in range(5):
            ...
            self.log(
                lambda idx: f"player{players[idx]} puts {card_to_str(np.array([decisions_arg[idx] // 13, decisions_arg[idx] % 13]))}."
            )
